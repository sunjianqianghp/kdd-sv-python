...
def getHottopicDevice(spark):
    sqlRequest = """select
                   ...
                   where c.day between '2019-07-15' and '2019-07-21'
                     and c.cmd = 137
    ...
    sql2 = """select
               ...
               join ( select device from hottopicDevice ) b
                 on a.uid = b.device
              where a.load_date between '2019-06-22' and '2019-07-21'
           """
    df1 = spark.sql(sql2)
...
def judge(appList):
    if "com.qukandian.video" in appList :
        if "com.jifen.qukan" not in appList:
            return 1
    return 0
...
if __name__ == "__main__":
    ...
    # result.show(20)
    # time.sleep(5)
    result.write.mode("overwrite").saveAsTable("test.hottopic_device_last3d_without_qtt_sjq") # 7698590
