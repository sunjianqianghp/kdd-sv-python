...
if __name__ == '__main__':
    ...
    sqlRequest = """select pv_id, device, groupid, props 
                    ...
                    where thedate = '2019-07-09'
                      and thehour between '16' and '19'
    ...
    df1.show(10)
    # df1.write.mode("overwrite").saveAsTable("test.basedata_for_auc2_sjq")
    ...
    df3 = df1.join(df2, ["device", "video_id"] ).filter("label is not null").select( "device", "groupid", "video_id", "score", "label")
    # df3.show()
    df3.write.mode("overwrite").saveAsTable("test.score_label2_sjq")
