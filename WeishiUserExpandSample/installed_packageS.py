...
def getUidInfoAppNum(spark):
    sql = """select
               ...
               join ( select device from test.hottopic_device_last7d_without_ws_sjq ) b
                 on a.uid = b.device
              where a.load_date between '2019-07-14' and '2019-07-23'
          """
    df = spark.sql(sql)
...
if __name__ == "__main__":
    spark = SparkSession.builder.appName("installed_packageS").enableHiveSupport().getOrCreate()
    uidInfoAppNum = getUidInfoAppNum(spark)
    uidInfoAppNum.write.mode("overwrite").saveAsTable("test.ws_uid_infoApp_numS_sjq") #7297633